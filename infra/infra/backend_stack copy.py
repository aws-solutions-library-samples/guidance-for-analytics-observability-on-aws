...
from aws_cdk import (
    Aspects, Aws, Stack, RemovalPolicy, CfnOutput, CfnTag, Fn, Duration, CustomResource, )
    ...
    def __init__(self, scope: Construct, construct_id: str, **kwargs) -> None:
        ...
        master_role.add_to_policy(
            PolicyStatement(
                ...
                resources=[f'arn:aws:es:{Aws.REGION}:{Aws.ACCOUNT_ID}:domain/{domain_name}'],
            ))
        master_role.add_to_policy(
            PolicyStatement(
                actions=[
        ...
        pipeline_policy = ManagedPolicy(self, 'PipelinePolicy',
                                        statements=[
                                            PolicyStatement(
                                                resources=[f"arn:aws:es:*:{stack.account}:domain/{domain_name}"],
        ...
        launch_config = CfnLaunchConfiguration(self, "RpLaunchConfig",
                                               ...
                                               image_id=ami,
                                               block_device_mappings=[CfnLaunchConfiguration.BlockDeviceMappingProperty(
                                                    device_name='volume',
                                                    # the properties below are optional
                                                    ebs=CfnLaunchConfiguration.BlockDeviceProperty(
...
"""),
        ...
        NagSuppressions.add_resource_suppressions_by_path(stack=self, path='/BackendStack/CustomResourcePolicy/Resource', suppressions=[
        ...
        NagSuppressions.add_resource_suppressions_by_path(stack=self, path='/BackendStack/MasterOpensearchRole/DefaultPolicy/Resource', suppressions=[
            {"id": "AwsSolutions-IAM5", "reason": "The ENI ID required by the custom resource for private subnet execution is not predictable"}
        ])

        NagSuppressions.add_resource_suppressions_by_path(stack=self, path='/BackendStack/PipelinePolicy/Resource', suppressions=[
        ...
        NagSuppressions.add_resource_suppressions_by_path(stack=self, path='/BackendStack/SparkObservabilityDomain', suppressions=[
        ...
        NagSuppressions.add_resource_suppressions_by_path(stack=self, path='/BackendStack/BootstrapLambdaProvider/framework-onEvent', suppressions=[
        ...
        NagSuppressions.add_resource_suppressions_by_path(stack=self, path='/BackendStack/LambdaRole/Resource', suppressions=[
            {"id": "AwsSolutions-IAM5", "reason": "The log stream of the AWS lambda custom resource is not predictable"},
            {"id": "AwsSolutions-IAM5", "reason": "Wildcard is required with ec2:List and ec2:Describe to find the EIP of the proxy"},
        ])

        NagSuppressions.add_resource_suppressions_by_path(stack=self, path='/BackendStack/RpSecurityGroup/Resource', suppressions=[
        ...
        NagSuppressions.add_resource_suppressions_by_path(stack=self, path='/BackendStack/RpIamRole/Resource', suppressions=[
            {"id": "AwsSolutions-IAM4", "reason": "AmazonEC2RoleforSSM is used because it's not possible to scope down permissions"},
        ])

        NagSuppressions.add_resource_suppressions_by_path(stack=self, path='/BackendStack/OsBootstrapFunction/Resource', suppressions=[
            {"id": "AwsSolutions-L1", "reason": "Python 3.11 is the latest version"},
        ])

        NagSuppressions.add_resource_suppressions_by_path(stack=self, path='/BackendStack/GetEC2PublicIP/Resource', suppressions=[
            {"id": "AwsSolutions-L1", "reason": "Python 3.11 is the latest version"},
        ])
